...
下列情况**不要继续纠结下载的 exe**，请改做 **[方式二：拉取源码并构建 exe](#方式二拉取源码并构建-exewindows)**（你已有仓库时，直接在项目根执行 `.\backend\scripts\build-standalone.ps1` 即可）：
...
方式二会在你本机用**当前源码 + 当前 `backend/model/`** 重新打包，成功率最高。若方式二构建也失败，把 PowerShell **完整报错**对照下文 [常见问题](#常见问题) 处理。
...
```text
backend/model/vosk-model-small-cn-0.22/
    conf/model.conf          ← 用于判断模型是否完整
    ...
```

- 若 `git clone` 后仓库里**已有** `backend/model/`（体积大，视仓库而定），检查上述 `conf/model.conf` 是否存在即可。
- 若没有：打开 [Vosk 模型列表](https://alphacephei.com/vosk/models)，下载 **Chinese small**（`vosk-model-small-cn-0.22`），解压到 `backend/model/`，保证路径与上表一致。
...
| 步骤 | 脚本 / 动作 | 说明 |
|------|-------------|------|
| 1 | `setup-vosk.ps1` | 准备 `backend/third_party/vosk/windows-amd64`；缺库时从 `backend/third_party/vosk-win64.zip` 或 [官方 zip](https://github.com/alphacep/vosk-api/releases/download/v0.3.45/vosk-win64-0.3.45.zip) 下载 |
| 2 | `prepare-bundle.ps1` | 复制 `frontend/web/`、`backend/model/vosk-model-small-cn-0.22` → `backend/internal/assets`，并写入内嵌默认 `default.yaml` |
| 3 | `go build ./cmd/server` | 生成 `backend/cmd/launcher/payload/speaknow-core.exe`（CGO，需 `gcc`） |
...
| 其它脚本 | 何时用 |
|----------|--------|
| `backend/scripts/build-standalone.ps1` | **默认用这个**（模型也打进 exe） |
| `backend/scripts/build-windows.ps1` | 不想把模型打进 exe、愿在运行目录保留 `backend/model/` 时用 |
...
| `setup-vosk` 下载失败 | 手动下载 [vosk-win64-0.3.45.zip](https://github.com/alphacep/vosk-api/releases/download/v0.3.45/vosk-win64-0.3.45.zip) 放到 `backend/third_party/`，再执行 `.\backend\scripts\setup-vosk.ps1` 后重新 `build-standalone` |
...
```powershell
.\backend\scripts\setup-vosk.ps1
```
...
开发模式下模型**不会**打进二进制，必须保留在 `backend/model/` 目录。

#### 4. 启动服务

```powershell
$env:CGO_ENABLED = "1"
go run -C backend ./cmd/server -config configs/config.yaml
```
...
| 网页 / 模型位置 | 内嵌在 exe，解压到 `.speaknow-data` | 直接读 `frontend/web/`、`backend/model/` |
...
| **构建报 CGO / gcc** | 安装 MinGW-w64，新开 PowerShell 再执行 `.\backend\scripts\build-standalone.ps1` |
...
| 项目 | 说明 |
|------|------|
| **推荐构建** | 已 clone 仓库 → [方式二](#方式二拉取源码并构建-exewindows) `.\backend\scripts\build-standalone.ps1` |
