...
#   sudo bash backend/deployments/install-linux.sh
#   sudo bash backend/deployments/install-linux.sh --no-build   # use existing bin/speaknow
set -euo pipefail
...
cp -a "$REPO_ROOT/../frontend/web/." "$INSTALL_PREFIX/web/"
